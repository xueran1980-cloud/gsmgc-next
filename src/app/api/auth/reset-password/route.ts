...
const RESET_PASSWORD_API = 'https://api.gsmgc.es/wp-json/gsmgc/v1/reset-password';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ct.includes('application/json')) {
      ...
      console.error('[API /auth/reset-password] Non-JSON response:', res.status, ct, text.slice(0, 200));
    ...
    return NextResponse.json(
      { success: false, message: 'Reset password proxy error', error: err.message },
      { status: 500 }
    );
  }
}
