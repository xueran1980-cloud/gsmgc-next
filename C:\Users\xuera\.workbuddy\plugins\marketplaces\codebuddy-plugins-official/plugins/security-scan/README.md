...
一款智能代码安全审计工具，通过 **语义索引 + 多 Agent 并行扫描 + 对抗验证** 实现专业级漏洞发现。支持 **全链路 --auto 无人值守模式 + 多维差异 POC 验证 + 安全门禁 + Git Hook 自动化 + 腾讯云安全基线**，覆盖代码安全与云安全的全链路审计。
...
**全项目扫描：**
```
/security-scan:project [--scan-level light|deep] [--include *.py,*.js] [--exclude node_modules] [--auto]
```

**Git 增量扫描：**
```
/security-scan:diff [--commit <hash>] [--scan-level light|deep] [--mode staged|unstaged|all] [--auto]
```
...
> `--auto` 模式跳过所有交互（权限白名单自动配置、模式选择、环境就绪降级、修复跳过），自动执行扫描、验证、门禁评估和通知。**安全红线**：绝不自动修复代码。
...
审计完成后自动生成 `poc-scripts.py`，采用**多维差异验证**架构（baseline 对比 + 布尔/时间/特征多维确认），可对目标服务发送实际探测请求验证漏洞：
