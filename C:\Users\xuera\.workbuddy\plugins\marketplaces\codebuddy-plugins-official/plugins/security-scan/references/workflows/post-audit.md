...
对满足生成条件（Critical/High 且 `confidence >= 90`）的 findings 生成可运行的 POC 验证脚本（`poc-scripts.py`）。

**三模式 POC 生成规则**：

| scan_mode | options.withPoc | 是否生成 POC |
|---|---|---|
| `deep` | 任意（默认 `false`） | **生成**（既有行为，不受 flag 影响） |
| `light` | 任意（默认 `false`） | **生成**（既有行为，不受 flag 影响） |
| `fast` | `false`（默认） | **跳过**（Fast 专属节流） |
| `fast` | `true`（命令行传 `--with-poc`） | **生成** |

**设计原则**：`--with-poc` flag 仅用于 Fast 模式下显式启用 POC。Light/Deep 的 POC 行为与历史版本一致，确保升级无感知。

**执行位置**：编排器在"修复阶段"根据 `batch-plan.json > scan_mode` 和 `options.withPoc` 决定是否跳过本步骤。

**判定示例**：

```bash
withPoc=$(python3 -c "import json; p=json.load(open('security-scan-output/${audit_batch_id}/batch-plan.json')); print('true' if p.get('options',{}).get('withPoc', False) else 'false')")
scanMode=$(python3 -c "import json; print(json.load(open('security-scan-output/${audit_batch_id}/batch-plan.json')).get('scan_mode','unknown'))")

# 只有 Fast + 未传 --with-poc 时才跳过；其他三种情况（Light、Deep、Fast+withPoc）都正常生成
if [ "$scanMode" = "fast" ] && [ "$withPoc" != "true" ]; then
  echo "[post-audit] Fast 模式未启用 --with-poc，跳过 POC 生成"
else
  # Light / Deep / Fast+withPoc：按既有流程生成 POC
  :
fi
```
...
> **`--auto` 模式**：跳过此交互，直接继续后续流程。未验证的高危漏洞信息会记录在报告中供后续人工复核。
...
> **`--auto` 模式**：跳过此交互，**绝不执行自动修复**，输出审计摘要后自动结束。auto 模式的安全红线是不修改用户代码。
