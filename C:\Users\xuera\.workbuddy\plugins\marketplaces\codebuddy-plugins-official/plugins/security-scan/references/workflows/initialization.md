...
| init-步骤5 环境就绪确认（pendingActions 不为空） | AskUserQuestion | **自动选择「跳过，继续扫描（降级模式）」**，不弹出交互 |
...
4. **降级不中断**：安装失败时降级继续，仅在需要用户手动操作时才提示（`--auto` 模式下自动选择降级继续）
  ...
  options:
    - label: "极速扫描（Fast）"
      description: |
        ...
        扫描与验证合并，默认跳过 POC。适合 Hook / IDE / CI 自动化场景。
...
> Fast 与 Light 在初始化阶段完全相同（均跳过 tree-sitter 和 LSP）。Fast 的差异从阶段 1 开始生效，见 `scan-mode.md > Fast 模式硬性约束`。
...
> **`--auto` 模式**：跳过 AskUserQuestion，自动选择「跳过，继续扫描（降级模式）」。输出 `[auto] 检测到 {len(pendingActions)} 个环境依赖未就绪，自动降级继续扫描。`，设置 `lspStatus = "unavailable"`，流水线继续。
