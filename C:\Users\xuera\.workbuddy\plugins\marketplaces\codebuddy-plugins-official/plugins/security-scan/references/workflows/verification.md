...
1. **阶段 2 LLM 内联**：Read Sink 上下文时同时完成 Source 可达性三判 + 代码存在性校验 + 防御检查（详见 `scan-mode.md > 阶段 2 Fast`），打标 `verificationStatus: "inline-verified" | "inline-dismissed"`。
