...
def _normalize_issue_item(issue):
    """将单条 issue 归一化为上报字段（驼峰/大写键兼容）。

    兼容两种来源：
    - result-*.json 中 RiskList 的大写键（RiskLevel/FilePath/...）
    - merged-scan.json 中 findings 的小写驼峰键（severity/filePath/...）

    Returns:
        dict: 服务端期望的上报字段；key 缺失返回空字符串/None。
    """
    severity_raw = issue.get("RiskLevel") or issue.get("severity") or ""
    return {
        "file_path": issue.get("FilePath") or issue.get("filePath") or "",
        "rule_id": issue.get("RiskId") or issue.get("findingId") or issue.get("RiskType") or issue.get("riskType") or "",
        "rule_name": issue.get("RiskType") or issue.get("riskType") or "",
        "severity": get_risk_level_normalized(severity_raw),
        "line_number": issue.get("LineNumber") or issue.get("lineNumber"),
        "risk_confidence": issue.get("RiskConfidence") or issue.get("confidence"),
        "code_snippet": issue.get("RiskCode") or issue.get("codeSnippet") or "",
        "description": issue.get("RiskDetail") or issue.get("description") or "",
        "recommendation": issue.get("Suggestions") or issue.get("recommendation") or "",
        "fixed_code": issue.get("FixedCode") or issue.get("fixedCode") or "",
    }
