...
def run_score(batch_dir):
    ...
    for f in all_verified:
        ...
        if fid and fid in merged_index:
            ...
            for key in ('riskType', 'RiskType', 'filePath', 'FilePath',
                        'lineNumber', 'LineNumber', 'severity', 'RiskLevel',
                        'attackChain', 'sourceAgent', 'traceMethod', 'codeSnippet'):
...
def run_challenge(batch_dir):
    ...
    try:
        for f in findings:
            ...
            # 越级且无 severityRationale → 强制下调到 severity_default
            severity_correction_record = None
            if severity_defaults and risk_type_slug:
                actual_sev = str(f.get('severity', '')).strip().lower()
                default_sev = severity_defaults.get(risk_type_slug)
                if default_sev and actual_sev:
                    actual_rank = SEVERITY_ORDER.get(actual_sev, 0)
                    default_rank = SEVERITY_ORDER.get(default_sev, 0)
                    has_rationale = bool(str(f.get('severityRationale', '')).strip())
                    if actual_rank > default_rank and not has_rationale:
                        # 越级且无合理升级说明 → 强制下调
                        severity_correction_record = {
                            'from': actual_sev,
                            'to': default_sev,
                            'reason': 'agent-rules.md §4 + taxonomy.severity_default',
                            'correctedBy': 'verifier.py:challenge',
                        }
                        f['severity'] = default_sev
                        f['severityCorrection'] = severity_correction_record
                        challenges.append({
                            'type': 'severity_violation',
                            'fromSeverity': actual_sev,
                            'toSeverity': default_sev,
