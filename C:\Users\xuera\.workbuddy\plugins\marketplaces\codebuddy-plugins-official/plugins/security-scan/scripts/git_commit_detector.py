...
# 配置读取
# ---------------------------------------------------------------------------
_DEFAULT_CONFIG = {"enabled": True, "scan_level": "fast", "blocking": False}
...
def _load_hook_config(cwd: str = "") -> dict:
    ...
    Returns:
        dict: {"enabled": bool, "scan_level": str, "blocking": bool}
    """
    try:
...
# 输出构建
# ---------------------------------------------------------------------------
def _build_silent_output(git_op: str, diff_args: str, scan_level: str) -> dict:
    """非阻断模式：直接指示 Agent 静默执行扫描，不询问用户。"""
    skill_name = "security-scan:diff"
    skill_args = f"{diff_args} --scan-level {scan_level} --auto"
    lines = [
        f"Security-Scan 插件检测到本轮对话执行了 git {git_op}。",
        f"根据项目 .codebuddy/security-scan/config.json 中的 auto_scan 配置，",
        f"请对本次变更执行增量安全扫描。",
        "",
...
def _build_blocking_output(git_op: str, diff_args: str, scan_level: str) -> dict:
    ...
    # 三个模式的固定描述文案
    _desc = {
        "fast": "极速扫描（Fast）— Light 的纪律化版本，并行 Read / 扫描验证合并 / 默认跳过 POC",
...
def _build_output(git_op: str, diff_args: str, config: dict) -> dict:
    ...
    return _build_silent_output(git_op, diff_args, config["scan_level"])
...
def main() -> None:
    ...
    _log(f"配置: enabled={config['enabled']}, scan_level={config['scan_level']}, blocking={config['blocking']}")
