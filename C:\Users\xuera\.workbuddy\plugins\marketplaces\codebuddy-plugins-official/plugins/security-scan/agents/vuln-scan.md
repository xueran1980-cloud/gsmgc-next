...
4. **Critical 仅限**：无认证直接 RCE / 已知恶意依赖 / 在野 CVE
5. **High 仅限**：SQLi/NoSQLi、auth-bypass、AKSK 泄漏、可 RCE 的调试端点、heapdump 类大量内存泄漏
6. **禁止**仅因「理论上可能」「最坏情况下」就提升到 Critical/High

**违反场景示例**（合并阶段会自动降级）：
- 把 weak-crypto / weak-password-hash / information-leak / log-injection / missing-security-audit 标 high → 强制降到 low
- 把 IDOR / CSRF / SSRF / XSS / hardcoded-secret 标 critical → 强制降到 medium
